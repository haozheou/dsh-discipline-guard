    ...
    try {
      ...
      if (cfg.costEnabled && !meter.userRelease) {
        ...
        const violation =
          ...
          meter.billed >= cfg.costTurnBudget;
        if (violation) {
          const detail = `上一步净计费输入 ${fmtTokens(meter.lastInput)}（缓存命中率 ${cachePct.toFixed(1)}%），本回合已累计 ${fmtTokens(meter.billed)} / 预算 ${fmtTokens(cfg.costTurnBudget)}`;
