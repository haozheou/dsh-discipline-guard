		...
		function GuardSettingsSection(props) {
			...
			return h(
				...
				h(
					...
					{
						title: "② 成本门（Cost）",
						help: "按每步真实 usage 计量：单步净计费输入超阈值且缓存命中率低于下限（token-plan 全价重发模式），或回合累计超预算 — 先注入预警要求汇报，同回合再犯则硬停回合；回复「继续」放行一次。「当前路由缓存判定」=auto 按命中率推断；选「有缓存」（如 deepseek 官方）则不因低命中率误报、只留预算硬顶；选「无缓存」（如 token-plan 套餐）则从严。"
					},
					h(Row, { label: "启用", kind: "switch", value: v.costEnabled, onChange: function (x) { set({ costEnabled: x }); } }),
					h(
						"div",
						{ className: "dg_row" },
						h("span", { className: "dg_rowLabel" }, "当前路由缓存判定"),
						h(
							"div",
							{ style: { display: "flex", gap: 8 } },
							[
								{ value: "auto", label: "auto · 自动推断" },
								{ value: "cached", label: "有缓存" },
								{ value: "nocache", label: "无缓存" }
							].map(function (opt) {
								return h(
									"button",
									{
										key: opt.value,
										type: "button",
										className: "dg_chipBtn",
										title:
											opt.value === "auto"
												? "按每步缓存命中率自动推断是否无缓存全价重发（默认）"
												: opt.value === "cached"
													? "当前路由已知有提示缓存（如 deepseek 官方）；成本门不再因低命中率误报，只保留回合预算硬顶"
													: "当前路由已知无缓存/套餐全价重发（如 token-plan 端点）；成本门从严"
									,
										style: v.routeCacheMode === opt.value ? { borderColor: "var(--dsw-alias-brand-primary)", color: "var(--dsw-alias-brand-primary)" } : {},
										onClick: function () { set({ routeCacheMode: opt.value }); }
									},
									opt.label
								);
							})
